...
class FotoController {
    ...
    public function procesarSubida() {
        ...
        // Inicializamos $gincana como false para evitar errores de "variable indefinida"
        $gincana = false;
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_FILES["foto"])) {
            ...
            // 1. COMPROBACIÓN DE LÍMITES GLOBALES
            ...
            // 2. COMPROBACIÓN DE GINCANA (Solo si hay etiqueta)
            if (!empty($etiqueta)) {
            ...
            // Redimensión y Compresión a 1920px (Optimización para Hosting)
            $max_res = 1920;
            ...
            if (imagejpeg($img_nueva, $ruta_destino, 80)) {
                ...
                if (!empty($etiqueta) && $gincana) {
                    // Si hay etiqueta y existe la gincana, vamos a su galería
                    header("Location: index.php?action=ver_gincana&etiqueta=" . urlencode($etiqueta));
                } else {
                    // Si no hay etiqueta o no es gincana oficial, vamos al perfil
